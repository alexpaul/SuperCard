...
#define CORNER_RADIUS 12.0 // of rounded rect
#define CORNER_FONT_SIZE 0.20 // 20% of card's width
#define TEXTBOUNDS_ORIGIN_X 2.0
#define TEXTBOUNDS_ORIGIN_Y 2.0
...
- (void)drawRect:(CGRect)rect
{
    ...
    //  This will clip all drawing to be inside the roundedRect 
    [roundedRect addClip];
...
- (void)drawCorners
{
    ...
    UIFont *cornerFont = [UIFont systemFontOfSize:self.bounds.size.width * CORNER_FONT_SIZE];
    ...
    CGRect textBounds;
    textBounds.origin = CGPointMake(TEXTBOUNDS_ORIGIN_X, TEXTBOUNDS_ORIGIN_Y);
...
{
    //  Code was changed in Lecture 6 19:06 left in video
    //UIGraphicsPushContext(UIGraphicsGetCurrentContext());
    
    CGContextRef context = UIGraphicsGetCurrentContext();
    CGContextSaveGState(context);
    CGContextTranslateCTM(context, self.bounds.size.width, self.bounds.size.height); //CTM context transformation matrix
    CGContextRotateCTM(context, M_PI); // angle in Radians
}
